...
5. frequentistLuminositySignificancePlots: This script uses the code from frequentist.py and creates three plots analyzing the local Z values, local p-values, and global Z values at different luminosities. 

6. bayesian.py: This script performs a Bayesian analysis of mass spectrum data, comparing background-only and signal-plus-background models using Dynesty for nested sampling. To use it, run the script, provide a CSV file with observed mass spectrum data, and it will output the results along with plots of the fit and posterior distributions for both models.

7. comparision.py: This script compares the frequentist results with the Bayesian results. It is used in datacomparision.py.
